			...
			<article id="post-<?php the_ID(); ?>" <?php post_class( 'clearfix' ); ?>>
				...
				<?php else : ?>
					...
					<div class="content">
						...
						<?php if ( $images ) : ?>
							<p><em><?php printf( _n( 'Dette galleri indeholder <a %1$s>%2$s billede</a>.', 'Dette galleri indeholder <a %1$s>%2$s billeder</a>.', $total_images, 'bike' ), 'href="' . esc_url( get_permalink() ) . '" title="' . esc_attr( sprintf( __( 'Direkte link til "%s"', 'bike' ), the_title_attribute( 'echo=0' ) ) ) . '"', number_format_i18n( $total_images ) ); ?></em></p>
						<?php endif; ?>
					</div>
				<?php endif; ?>
			</article>
